...
async function register(req, res, next) {
  try {
    ...
    return res.status(201).json({
      ...
      tokens: { accessToken, refreshToken, expiresIn: process.env.ACCESS_TOKEN_EXPIRES_IN || '15m' }
    });
  } catch (err) {
...
async function refresh(req, res, next) {
  try {
    ...
    return res.status(200).json({
      tokens: { accessToken, refreshToken: newRefreshToken, expiresIn: process.env.ACCESS_TOKEN_EXPIRES_IN || '15m' }
    });
  } catch (err) {
